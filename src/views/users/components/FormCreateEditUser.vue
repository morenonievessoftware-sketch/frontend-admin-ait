...
const emit = defineEmits<{
  ...
  (event: "isNewUser", value: boolean): void
}>()

const cerrarModal = (event: any) => {
  emit("update:visible", event)
  emit("isNewUser", false)
  resetForm()
}
...
watch(
  () => [props.visible, props.isNewUser],
  ([newVisible, newIsNewUser]) => {
    if (newVisible && newIsNewUser) {
      findUserById()
      resetForm()
    }
  },
...
<template>
  <Dialog :visible="props.visible" modal :style="{ width: '50rem' }" :draggable="false" @update:visible="cerrarModal"
    ...
    <form @submit="createUser" class="space-y-6">
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div class="space-y-2">
          ...
          <input id="nombre" v-model.trim="nombre" v-bind="nombreAttrs" type="text" placeholder="Ingresa el nombre"
            :class="[inputBase, errors.nombre ? inputErr : inputOk]" />
        ...
        <div class="space-y-2">
          ...
          <input id="email" v-model.trim="email" v-bind="emailAttrs" type="email" placeholder="correo@dominio.com"
        ...
        <div class="space-y-2">
          ...
          <select id="rol" v-model="rol" v-bind="rolAttrs" :class="[inputBase, errors.rol ? inputErr : inputOk]">
        ...
        <div class="space-y-2">
          ...
          <input id="institucion" v-model.trim="institucion" v-bind="institucionAttrs" type="text"
            placeholder="Nombre de la institución" :class="[inputBase, errors.institucion ? inputErr : inputOk]" />
